...
class DataFrameTransform:
    ...
    def impute_nulls(self, columns, method='median'):
        # Imputes NULL values in specified columns using median or mean
        for col in columns:
            if method == 'median':
    ...
    def identify_skewed_columns(self, threshold=0.75):
        # Identifies columns with skewness above the threshold
    ...
    def transform_skewed_columns(self, columns):
        # Transforms skewed columns using appropriate methods
        for col in columns:
    ...
    def identify_highly_correlated_columns(self, threshold=0.8):
        # Computes correlation matrix and identify highly correlated columns
